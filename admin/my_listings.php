...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      while ($fetch_images = $select_images->fetch(PDO::FETCH_ASSOC)) {
         ...
         $imagePath_01 = '../uploaded_files/' . $image_01;
         $imagePath_02 = '../uploaded_files/' . $image_02;
         $imagePath_03 = '../uploaded_files/' . $image_03;
         $imagePath_04 = '../uploaded_files/' . $image_04;
         $imagePath_05 = '../uploaded_files/' . $image_05;
       
         if (file_exists($imagePath_01)) {
           unlink($imagePath_01);
         }
         if (!empty($image_02) && file_exists($imagePath_02)) {
           unlink($imagePath_02);
         }
         if (!empty($image_03) && file_exists($imagePath_03)) {
           unlink($imagePath_03);
         }
         if (!empty($image_04) && file_exists($imagePath_04)) {
           unlink($imagePath_04);
         }
         if (!empty($image_05) && file_exists($imagePath_05)) {
           unlink($imagePath_05);
         }
      ...
      $delete_listing = $conn->prepare("DELETE FROM `explore` WHERE id = ?");
...
<section class="my-listings">
   ...
   <?php
      $total_images = 0;
      $select_properties = $conn->prepare("SELECT * FROM `explore` WHERE admin_id = ? ORDER BY date DESC");
      $select_properties->execute([$admin_id]);
   ...
   <form accept="" method="POST" class="box">
      ...
      <a href="view_propertyadmin.php?get_id=<?= $property_id; ?>" class="btn">view property</a>
   </form>
   <?php
         }
      }else{
