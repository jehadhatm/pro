...
if (isset($_POST['send'])) {
   ...
   $insert_report = $conn->prepare("INSERT INTO `reports` (property_id, user_id, report_type) VALUES (?, ?, ?)");
   $insert_report->execute([$property_id, $user_id, $report_type]);

   // Get the automatically generated report_id
   $report_id = $conn->lastInsertId();

   $success_msg[] = 'Report submitted successfully! Report ID: ' . $report_id;
}
?>